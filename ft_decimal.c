...
/*   Updated: 2017/03/20 18:41:49 by bmoiroud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_print		*ft_intflags(t_print *p, char *nbr)
{
	...
	if (p->sign == 0 && p->leftalign != 1 && pr > fw)
		...
		while (i++ != fw - p->nlen && fw > p->nlen)
			ft_putbuffer(p, " ", 1);
	p = ft_intflag(p, nbr, sign, 0);
	i = -1;
	if (p->leftalign && fw > p->nlen && pr < fw)
		while (++i <= fw - ((pr > p->nlen) ? pr : p->nlen) - p->sign % 2 - 1)
			ft_putbuffer(p, " ", 1);
	else if (p->leftalign && fw > p->nlen && pr > fw)
		while (++i <= pr - fw - p->nlen)
			ft_putbuffer(p, " ", 1);
	return (p);
}
